"""
JVS corpus gold layer processing.

Gold = "what exactly is the dataset I will train on, with stable splits
and a stable snapshot id?"

Note: JVS has 100 speakers, so splits are computed at the utterance level
(not speaker level) for simplicity. Future work may want speaker-stratified splits.

Command: koe gold jvs

Reads:
    lake/silver/jvs/utterances (Delta table)

Writes:
    lake/gold/jvs/utterances (Delta table)
    lake/gold/jvs/manifests/{snapshot_id}.jsonl (training manifest)
"""
...
def read_silver_jvs(spark: SparkSession) -> DataFrame:
    """
    Read JVS silver table.
    ...
    """
    silver_path = paths.silver / "jvs" / "utterances"

    if not silver_path.exists():
        raise FileNotFoundError(
            f"JVS silver table not found: {silver_path}\n"
            "Run silver first: koe silver jvs"
        )
...
def filter_trainable(
    ...
    # Apply filters: is_trainable + duration bounds + has transcript
    # JVS may have some utterances without transcripts
    filtered = df.filter(
    ...
    excluded_no_text = df.filter(F.col("text_raw").isNull()).count()
    ...
    stats = {
        ...
        "excluded_no_transcript": excluded_no_text,
    }
...
def use_silver_split_or_fallback(
    ...
    if null_split_count == 0:
        ...
        raise ValueError(
            ...
            "Re-run silver pipeline to fix: koe silver jvs --force"
        )
...
def build_gold_df(
    ...
    silver_df = read_silver_jvs(spark)
    silver_count = silver_df.count()
    print(f"  Silver records: {silver_count}")

    # Filter to trainable
    print(f"Filtering (duration {min_duration}-{max_duration}s, is_trainable=true, has text)...")
    ...
    if filter_stats['excluded_no_transcript'] > 0:
        print(f"    (no transcript: {filter_stats['excluded_no_transcript']})")
    ...
    gold_df = bucket_df.select(
        ...
        # Phonemes - JVS has corpus_provided phonemes in bronze
        F.coalesce(
...
def build_gold_jvs(
    ...
    This is the main entry point for `koe gold jvs`.
    """
    print("=" * 60)
    print("JVS Gold Pipeline")
    print("=" * 60)
    ...
    if snapshot_id is None:
        ...
        snapshot_id = generate_snapshot_id("jvs", config_hash)
    ...
    # Write Delta table
    output_path = paths.gold / "jvs" / "utterances"
    if write_delta:
    ...
    if export_jsonl:
        ...
        if manifest_out:
            ...
            manifests_dir = paths.gold / "jvs" / "manifests"
    ...
    print("\n" + "=" * 60)
    print("JVS Gold Complete")
    print("=" * 60)
    print(f"\nSnapshot: {snapshot_id}")
    print(f"Seed: {seed}")
    print(f"Speakers: {val_stats['speaker_count']}")
